            ...
            <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-50/70 via-orange-50/40 to-amber-50/70 dark:from-amber-950/20 dark:via-orange-950/10 dark:to-amber-950/20 border border-amber-200/80 dark:border-amber-900/40 space-y-3 shadow-2xs">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="text-xs font-black uppercase text-amber-900 dark:text-amber-200 tracking-wider">
                      Room & Sub-Unit Utility Cost Allocation
                    </h4>
                    <p className="text-[11px] text-amber-700/80 dark:text-amber-300/80">
                      Live tracking of electricity tokens and utility spend allocated to individual rooms in {unit.name}.
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-xs font-mono font-bold text-amber-900 dark:text-amber-200 bg-white dark:bg-slate-900 px-3 py-1 rounded-xl border border-amber-200 dark:border-amber-800/80 shadow-2xs">
                  <span>Facility Utility Total:</span>
                  <span className="font-extrabold text-amber-700 dark:text-amber-400">
                    {formatZAR(unitUtilities.reduce((sum, u) => sum + (u.amountZAR || 0), 0))}
                  </span>
                </div>
              </div>
              ...
              {(() => {
                ...
                unitUtilities.forEach(u => {
                  ...
                  if (!roomMap[rName]) {
                    const roomOccupants = employees
                      .filter(e => e.accommodationDetails?.campId === unit.id && (
                  ...
                  roomMap[rName].totalAmount += u.amountZAR;
                  roomMap[rName].count += 1;
                  roomMap[rName].types.add(u.utilityType.split('/')[0].trim());
                });
                      ...
                      >
                        <div>
                          <div className="flex items-center justify-between gap-1">
                            ...
                            <span className="text-[10px] text-slate-400 font-mono shrink-0">{r.count} expense{r.count > 1 ? 's' : ''}</span>
                          </div>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
                          <span className="text-[10px] text-slate-400 truncate max-w-[120px]">{Array.from(r.types).join(', ')}</span>
          ...
          <Card className="w-full max-w-xl bg-white dark:bg-slate-900 shadow-2xl border border-slate-200 dark:border-slate-800 rounded-2xl animate-in fade-in zoom-in-95 my-auto max-h-[92vh] flex flex-col">
            ...
            <form onSubmit={handleSaveUtility} className="p-6 space-y-4 overflow-y-auto flex-1 text-slate-900 dark:text-slate-100">
              <div>
                ...
                {(() => {
                  const rCount = unit.totalRooms || 0;
                  return (
