...
import { LabourAllocation } from '../types';
...
export function LabourAllocationView({ projectId }: { projectId: string }) {
  const { labourAllocations, activities, addLabourAllocation, updateLabourAllocation, deleteLabourAllocation, userRole } = useAppContext();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingId) {
      updateLabourAllocation(formData as LabourAllocation);
    } else {
      addLabourAllocation({
        ...formData,
